package Logos::Generator::libhooker::Method;
use strict;
...
sub initializers {
	...
	if(!$method->isNew) {
		my $r = "LBHookMessage(".$classvar.", ".$self->selectorRef($method->selector).", (void *)&".$self->newFunctionName($method).", (void *)&".$self->originalFunctionName($method).");";
	} else {
		my $r = "";
		$r .= "{ ";
		if(!$method->type) {
